...
public class AVLTree < E
    ...
    private AVLNode < E > delete(AVLNode < E > localRoot, E target) {
        ...
        if (target.compareTo(localRoot.data) == 0) {
            ...
            // silinmek istenen deger local root'tan kucukse
            // local root'un soluyla recursive cagri yap
            localRoot.left = delete( (AVLNode < E > ) localRoot.left, target);
            if (decrease) {
                incrementBalance(localRoot);
                // tree right heavy ise right-left rebalance yap
                if (localRoot.balance > AVLNode.RIGHT_HEAVY)
                    return rebalanceRightL(localRoot);
                // return local root otherwise
                return localRoot;
            }
            else
                return localRoot;
            ...
            // silinmek istenen deger local root'tan buyukse
            // local root'un sagiyla recursive cagri yap
            localRoot.right = delete( (AVLNode < E > ) localRoot.right, target);
            if (decrease) {
                decrementBalance(localRoot);
                // tree left heavy ise left-right rebalance yap
